...
def _parse_dt(raw: str) -> Optional[str]:
    """Parse a date string → ISO 8601, or None."""
    raw = raw.strip()
    if not raw:
        return None
...
def _uuid(raw: str) -> str:
    """Return raw if it's already a valid UUID, otherwise generate a new one."""
    raw = raw.strip()
    if raw:
        try:
...
def _norm(row: dict) -> dict:
    """Normalize a CSV row's keys to lowercase + stripped."""
...
def parse_companies_csv(content: str) -> tuple[list[dict], list[str]]:
    """Parse a Companies sheet CSV export.

    Returns (rows_to_upsert, errors).
    Each row dict is ready to pass to supabase.table("companies").upsert().
    """
    rows: list[dict] = []
    errors: list[str] = []

    reader = csv.DictReader(io.StringIO(content.lstrip("\ufeff")))  # strip BOM
...
def parse_people_csv(content: str) -> tuple[list[dict], list[str]]:
    """Parse a People sheet CSV export.

    Returns (rows_to_upsert, errors).
    Rows without an email are skipped since email is the unique business key.
    """
    rows: list[dict] = []
    errors: list[str] = []
    ...
    for i, raw_row in enumerate(reader, start=2):
        ...
        try:
            rows.append({
                ...
                "email":              email,
                "phone":              _str(row.get("phone", "")),
                # Google Sheets exports "linkedIn" header — handle both casings
...
def parse_demos_csv(content: str) -> tuple[list[dict], list[str]]:
    """Parse a Demos sheet CSV export.

    Returns (rows_to_upsert, errors).
    Rows without people_id are skipped (orphaned demo is meaningless).
    """
    rows: list[dict] = []
    errors: list[str] = []
