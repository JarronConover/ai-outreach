...
class ProspectingAgent:
    ...
    def _build_agent_with_exclusions(self, existing_emails: list = None):
        """Build an agent with existing emails injected into the system prompt."""
        system_prompt = _SYSTEM_PROMPT
        if existing_emails:
            excluded_list = "\n".join([f"- {email}" for email in existing_emails])
            system_prompt += f"\n\nIMPORTANT: Do NOT include these people (already in the system):\n{excluded_list}"
        return create_agent(self._llm, [self._search_tool], system_prompt=system_prompt)
